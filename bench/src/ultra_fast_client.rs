...
impl UltraFastBenchClient {
    ...
    pub async fn new(base_url: &str) -> Result<Self> {
        ...
        let pool = Pool::builder()
            ...
            .build(TcpConnectionManager::new(format!(
                "{}:{}",
                host, binary_port
            )))
            .await?;
    ...
    async fn lookup_batch_binary(
        ...
        for chunk in response_data.chunks_exact(17) {
            // 8 + 1 + 8 = 17 bytes per result
    ...
    async fn put_batch_http_fallback(&self, items: &[(u64, &[u8])]) -> Result<Vec<u64>> {
        ...
        for chunk in items.chunks(chunk_size) {
            let chunk_futures: Vec<_> = chunk
                .iter()
                .map(|&(key, value)| self.put_http_fallback(key, value))
                .collect();
...
impl UltraFastBenchClient {
    ...
    pub async fn benchmark_simd_batch(
        &self,
        keys: &[u64],
        iterations: usize,
    ) -> Result<SIMDBenchmarkResults> {
