...
fn bench_raw_indexes(c: &mut Criterion) {
    let n = std::env::var("KYRO_BENCH_N")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(5_000_000usize);
    // Use full working set by default so we don't overfit to warmed caches
    let sample = std::env::var("KYRO_BENCH_SAMPLES")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(n);
    // Optional cache-busting to induce LLC/dTLB misses (MiB)
    let cache_bust_mb = std::env::var("KYRO_BENCH_CACHE_BUST_MB")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0usize);
    // Simulated value size (bytes) and arena memory to map offsets into (MiB)
    let val_bytes = std::env::var("KYRO_BENCH_VAL_BYTES")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(64usize);
    let arena_mb = std::env::var("KYRO_BENCH_VALUE_ARENA_MB")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(512usize);
    // Hotset parameters to mimic skew: fraction of accesses and size of hot set
    let hot_frac: f64 = std::env::var("KYRO_BENCH_HOT_FRAC")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.2);
    let hot_set_frac: f64 = std::env::var("KYRO_BENCH_HOT_SET_FRAC")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.01);
    ...
    RmiIndex::write_from_pairs_v5(&v5_tmp, &pairs, pack_u32).unwrap();
    ...
    let mut poison: Vec<u8> = if cache_bust_mb > 0 {
        vec![0u8; cache_bust_mb * 1024 * 1024]
    } else {
        Vec::new()
    };
    let mut poison_idx: usize = 0;
    let mut arena: Vec<u8> = if arena_mb > 0 {
        vec![0u8; arena_mb * 1024 * 1024]
    } else {
        vec![0u8; 1]
    };
    ...
    fn cache_bust(buf: &mut [u8], idx: &mut usize) {
        ...
        for _ in 0..256 {
            let i = *idx;
            unsafe {
                *buf.get_unchecked_mut(i) = buf.get_unchecked(i).wrapping_add(1);
            }
            *idx = (i + step) % buf.len();
        }
    }
    ...
    fn simulate_value_read(arena: &mut [u8], base_off: u64, val_bytes: usize, val_stride: usize) {
        ...
        let start = ((base_off as usize).wrapping_mul(val_stride))
            % (len.saturating_sub(val_stride).max(1));
        // Touch first byte in each cache line up to val_bytes
        let mut off = start;
        let end = start + val_bytes.min(val_stride);
        while off < end {
            unsafe {
                let v = *arena.get_unchecked(off);
                black_box(v);
            }
            off += 64;
        }
    ...
    group.bench_function(BenchmarkId::from_parameter("btree_plus_value"), |b| {
        ...
        b.iter(|| {
            ...
            let k = keys[i % keys.len()];
            i = i.wrapping_add(1);
            if let Some(off) = bt.get(&k) {
                simulate_value_read(&mut arena, off, val_bytes, val_stride);
            }
        })
    });
    ...
    group.bench_function(BenchmarkId::from_parameter("rmi_full_v4_plus_value"), |b| {
        ...
        b.iter(|| {
            ...
            let k = keys[i % keys.len()];
            i = i.wrapping_add(1);
            if let Some(off) = rmi_v4.predict_get(&k) {
                simulate_value_read(&mut arena, off, val_bytes, val_stride);
            }
        })
    });
    ...
    group.bench_function(BenchmarkId::from_parameter("rmi_full_v5_plus_value"), |b| {
        ...
        b.iter(|| {
            ...
            let k = keys[i % keys.len()];
            i = i.wrapping_add(1);
            if let Some(off) = rmi_v5.predict_get(&k) {
                simulate_value_read(&mut arena, off, val_bytes, val_stride);
            }
        })
    });
