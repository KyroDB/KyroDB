...
fn test_basic_integration_pipeline() {
    ...
    let trained_events = training_pipeline(&mut logger, &mut predictor)
        .expect("Training pipeline failed");
...
fn test_time_windowed_training() {
    ...
    assert!(recent.len() <= all_events.len(), "Recent should be subset of all");
...
fn test_cache_admission_policy() {
    ...
    assert!(doc1_admitted, "Hot document (100 accesses) should be cached");
...
fn test_high_volume_integration() {
    ...
    let trained_events = training_pipeline(&mut logger, &mut predictor)
        .expect("High-volume training failed");
