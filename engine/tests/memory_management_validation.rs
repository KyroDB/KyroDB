...
async fn test_memory_management_oom_prevention() {
                ...
                match rmi_clone.insert(key, value) {
                    Ok(_) => {
                        ...
                        if let Some(retrieved) = rmi_clone.lookup(key) {
                            if retrieved != value {
                                panic!("Data integrity violation: key={}, expected={}, got={}", 
                                    key, value, retrieved);
                            }
                        }
                    }
                    Err(e) => {
                        ...
                        if error_msg.contains("memory pressure") || 
                           ...
                           error_msg.contains("rejected") {
                            ...
                            let backoff_ms = std::cmp::min(100, 1 + (rejections.load(Ordering::Relaxed) / 100));
                            if backoff_ms > 1 {
    ...
    assert!(final_successes > 10_000, "Should successfully insert at least 10K items");
    assert!(final_attempts == 400_000, "Should attempt all 400K inserts (8 threads × 50K each)");
    ...
    println!("   Final state: segments={}, hot_buffer={}, overflow={}", 
    ...
    assert!(final_stats.segment_count > 0, "Should have created segments");
    ...
    println!("   Data integrity: {}/{} checks passed", integrity_checks - integrity_failures, integrity_checks);
    assert_eq!(integrity_failures, 0, "Should have zero data integrity violations");
...
async fn test_memory_management_back_pressure_validation() {
    ...
    loop {
        ...
        match rmi.insert(key, value) {
            Ok(_) => {
                ...
                if insert_count % 5000 == 0 {
                    ...
                    println!("   Progress: {} inserts, hot={}, overflow={}, segments={}", 
                        insert_count, stats.hot_buffer_size, stats.overflow_size, stats.segment_count);
                }
            }
            Err(e) => {
                let error_msg = e.to_string();
                if error_msg.contains("memory pressure") || 
                   ...
                   error_msg.contains("Circuit breaker") {
                    ...
                    if rejection_count.load(Ordering::Relaxed) >= 10 {
                        println!("✅ Back-pressure activated after {} successful inserts", insert_count);
                        break;
                    }
        ...
        if start_time.elapsed() > Duration::from_secs(30) {
            println!("⚠️  Test timeout - system handled {} inserts without back-pressure", insert_count);
            break;
        }
        
        if insert_count > 200_000 {
            println!("✅ System handled {} inserts - excellent capacity!", insert_count);
            break;
        }
    }
    ...
    println!("   Final state: segments={}, hot={}, overflow={}", 
        final_stats.segment_count, final_stats.hot_buffer_size, final_stats.overflow_size);
    ...
    assert!(back_pressure_worked || high_capacity_achieved, 
        "Either back-pressure should activate OR system should handle high load efficiently");
    
    // Data should exist in the system
    assert!(final_stats.total_keys > 0, "Should have stored data successfully");
