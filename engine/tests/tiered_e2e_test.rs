...
fn test_three_tier_query_journey() {
    ...
    assert!(result.is_some(), "Doc 101 should be in cold tier after flush");
    
    let stats = engine.stats();
    println!("Cold tier searches after flush: {}", stats.cold_tier_searches);
    assert!(stats.cold_tier_searches > 5, "Cold tier searches should increase after flush");
    ...
    let stats = engine.stats();
    println!("Cache hits: {}, Total queries: {}", stats.cache_hits, stats.total_queries);
    
    // After multiple queries, some should hit cache
    // Note: Cache admission depends on strategy, so we just verify it's being used
    assert!(stats.cache_hits > 0 || stats.cold_tier_searches > 0, "Cache should be active");
    ...
    for (i, result) in results.iter().enumerate() {
        println!("  {}. doc_id={}, distance={:.4}", i+1, result.doc_id, result.distance);
    }
...
fn test_persistence_across_all_tiers() {
    ...
    // Phase 2: Recover and verify
    {
        ...
        // Verify initial docs
        assert!(recovered.query(0, None).is_some(), "Doc 0 should be recovered");
        assert!(recovered.query(1, None).is_some(), "Doc 1 should be recovered");
        assert!(recovered.query(2, None).is_some(), "Doc 2 should be recovered");
        
        // Verify inserted docs
        assert!(recovered.query(10, None).is_some(), "Doc 10 should be recovered");
        assert!(recovered.query(11, None).is_some(), "Doc 11 should be recovered");
        assert!(recovered.query(20, None).is_some(), "Doc 20 should be recovered");
        assert!(recovered.query(21, None).is_some(), "Doc 21 should be recovered");
        
        println!("All documents recovered successfully");
        
        // Hot tier should be empty after recovery (ephemeral)
        assert_eq!(recovered.hot_tier().len(), 0, "Hot tier should be empty after recovery");
