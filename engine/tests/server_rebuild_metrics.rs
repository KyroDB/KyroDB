...
async fn server_background_rebuild_emits_metrics() {
    ...
    let mut child = tokio::process::Command::new(kyro)
        ...
        .arg("127.0.0.1")
        .arg(port.to_string())
        .arg("--rmi-rebuild-appends")
        .arg("50")
        .arg("--wal-segment-bytes")
        .arg("1048576")
        .kill_on_drop(true)
        .spawn()
        .expect("spawn server");
    ...
    while attempts < 50 {
        ...
        if let Ok(text) = client.get(metrics_url).send().await.unwrap().text().await {
            if text.contains("kyrodb_rmi_rebuilds_total")
                && text.contains("kyrodb_rmi_rebuild_duration_seconds_bucket")
            {
                // check count >= 1
                if text.lines().any(|l| {
                    l.starts_with("kyrodb_rmi_rebuilds_total ")
                        && l.split_whitespace().last().unwrap_or("0") != "0"
                }) {
                    saw_rebuild = true;
                    break;
                }
