...
mod integration_tests {
    ...
    async fn test_health_endpoint() {
        ...
        let resp = timeout(
            TEST_TIMEOUT,
            client.get(&format!("{}/health", BASE_URL)).send()
        ).await.expect("Request timed out").expect("Request failed");
    ...
    async fn test_append_endpoint() {
        ...
        let resp = timeout(
            TEST_TIMEOUT,
            client.post(&format!("{}/v1/append", BASE_URL))
                .json(&payload)
                .send()
        ).await.expect("Request timed out").expect("Request failed");
    ...
    async fn test_kv_put_and_lookup() {
        ...
        let put_resp = timeout(
            TEST_TIMEOUT,
            client.post(&format!("{}/v1/put", BASE_URL))
                .json(&put_payload)
                .send()
        ).await.expect("Request timed out").expect("Request failed");
        ...
        let lookup_resp = timeout(
            TEST_TIMEOUT,
            client.get(&format!("{}/v1/lookup?key={}", BASE_URL, test_key)).send()
        ).await.expect("Request timed out").expect("Request failed");
        
        assert_eq!(lookup_resp.status(), 200);
        let lookup_body: serde_json::Value = lookup_resp.json().await.expect("Invalid JSON response");
    ...
    async fn test_snapshot_endpoint() {
        ...
        assert!(resp.status() == 200 || resp.status() == 429, 
                "Expected 200 or 429, got {}", resp.status());
    ...
    async fn test_sql_endpoint() {
        ...
        let insert_resp = timeout(
            TEST_TIMEOUT,
            client.post(&format!("{}/v1/sql", BASE_URL))
                .json(&insert_sql)
                .send()
        ).await.expect("Request timed out").expect("Request failed");
        
        assert_eq!(insert_resp.status(), 200);
        let insert_body: serde_json::Value = insert_resp.json().await.expect("Invalid JSON response");
        ...
        let select_body: serde_json::Value = select_resp.json().await.expect("Invalid JSON response");
    ...
    async fn test_fast_endpoints() {
        ...
        let lookup_resp = timeout(
            TEST_TIMEOUT,
            client.get(&format!("{}/v1/lookup_fast/{}", BASE_URL, test_key)).send()
        ).await.expect("Request timed out").expect("Request failed");
        
        if lookup_resp.status() == 200 {
            let bytes = lookup_resp.bytes().await.expect("Failed to get response bytes");
        ...
        let get_resp = timeout(
            TEST_TIMEOUT,
            client.get(&format!("{}/v1/get_fast/{}", BASE_URL, test_key)).send()
        ).await.expect("Request timed out").expect("Request failed");
        
        if get_resp.status() == 200 {
            let value_bytes = get_resp.bytes().await.expect("Failed to get response bytes");
