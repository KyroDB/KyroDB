...
async fn test_unbounded_memory_growth_fixed() {
    ...
    for i in 0..5000 {
        ...
        if i % 100 == 0 {
            ...
            println!("Progress: {} writes, {} successful, {} rejected, {} hot buffer size", 
    ...
    assert!(successful_writes > 100, "Should have some successful writes");
    ...
    assert!(final_stats.hot_buffer_size < 10000, "Hot buffer should be bounded");
    assert!(final_stats.overflow_size < 10000, "Overflow buffer should be bounded");
...
async fn test_adaptive_background_timing() {
    ...
    for cycle in 0..5 {
        ...
        let stats = rmi.get_stats();
        println!("Cycle {}: segments={}, total_keys={}, merge_in_progress={}", 
    ...
    println!("  Hot buffer utilization: {:.2}", final_stats.hot_buffer_utilization);
    
    // System should have processed the data and created segments
    assert!(final_stats.total_keys > 0, "Should have processed some data");
    
    // Background maintenance should have run without crashing
    assert!(elapsed >= Duration::from_secs(2), "Test should have run for reasonable time");
...
async fn test_insert_bounded_behavior() {
        ...
        match rmi.insert(i, i * 3) {
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "Error should indicate pressure/bounded behavior: {}", error_msg
                );
    ...
    assert!(first_rejection_count.is_some(), "Should eventually reject inserts");
    ...
    assert!(stats.hot_buffer_size < 5000, "Hot buffer should stay bounded");
    assert!(stats.overflow_size < 5000, "Overflow buffer should stay bounded");
