...
async fn build_and_check(dist_keys: Vec<u64>) {
    ...
    for (i, (k, _)) in pairs.iter().enumerate() {
        if i % 100 == 0 { cnt += 1; assert!(log2.lookup_key(*k).await.is_some(), "missing key {}", k); if cnt >= 100 { break; } }
    }
}
...
async fn fuzz_zipf_like_keys() {
    ...
    for i in 0..50_000u32 {
        if i % 1000 == 0 { keys.push(uni_large.sample(&mut rng)); } else { keys.push(uni_small.sample(&mut rng)); }
    }
    keys.sort(); keys.dedup();
    build_and_check(keys).await;
}
