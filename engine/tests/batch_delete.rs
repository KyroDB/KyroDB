use kyrodb_engine::{
    ...
    persistence::FsyncPolicy,
    HotTier, HnswBackend, TieredEngine, TieredEngineConfig,
    QueryHashCache,
};
...
fn test_tiered_engine_batch_delete_by_filter() {
    ...
    let mut m0 = HashMap::new(); m0.insert("cat".to_string(), "A".to_string());
    let mut m1 = HashMap::new(); m1.insert("cat".to_string(), "B".to_string());
    ...
    let mut m2 = HashMap::new(); m2.insert("cat".to_string(), "A".to_string());
    let mut m3 = HashMap::new(); m3.insert("cat".to_string(), "C".to_string());
    
    engine.insert(2, vec![2.0], m2).unwrap();
    engine.insert(3, vec![3.0], m3).unwrap();
    
    // Delete where cat=A (should delete 0 and 2)
    let count = engine.batch_delete_by_filter(|meta| {
        meta.get("cat").map(|v| v == "A").unwrap_or(false)
    }).unwrap();
