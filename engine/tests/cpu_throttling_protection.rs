...
async fn test_cpu_pressure_adaptive_intervals() {
    ...
    println!("✅ Background task adapted intervals over {} measurements", operation_intervals.len());
    ...
    if operation_intervals.len() >= 3 {
        ...
        let variance_detected = operation_intervals.iter()
            .any(|&interval| interval != first_interval);
        
        assert!(variance_detected || operation_intervals.len() < 5, 
...
async fn test_emergency_mode_activation() {
    ...
    match result {
        ...
        Err(_) => {
            let completed_ops = stress_operations.load(Ordering::Relaxed);
            println!("⏰ Stress test timed out after {} operations (expected under high pressure)", completed_ops);
...
async fn test_comprehensive_cpu_throttling_protection() {
    ...
    println!("📈 Baseline: {} ops in {:?} = {:.1} ops/sec", 
    ...
    println!("📉 Throttled: {} ops in {:?} = {:.1} ops/sec", 
    ...
    println!("📊 Recovery: {} ops in {:?} = {:.1} ops/sec", 
    ...
    assert!(baseline_operations >= 135, 
        "Baseline should complete most operations: {}/150", baseline_operations);
    assert!(throttled_ops > 0, 
        "Should complete some operations even when throttled: {}", throttled_ops);
    assert!(recovery_operations >= 80, 
        "Recovery should restore high completion rate: {}/100", recovery_operations);
    
    // Validate throughput recovery (more lenient for CI environments)
    let recovery_ratio = recovery_throughput / baseline_throughput;
    assert!(recovery_ratio > 0.6, 
        "Recovery throughput should be reasonable vs baseline: {:.2}", recovery_ratio);
    ...
    for i in 0..5 {
        ...
        assert!(lookup_result.is_some() || lookup_result.is_none(), 
