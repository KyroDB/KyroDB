...
async fn test_router_generation_consistency_under_concurrent_modifications() {
    ...
    assert!(total_successful > 500, "Should have many successful lookups, got {}", total_successful);
...
async fn test_epoch_based_segment_update_validation() {
                    ...
                    Ok(_) => {
                        ...
                        if let Some(read_value) = rmi_clone.lookup(key) {
                            if read_value != value {
                                ...
                                eprintln!("Data inconsistency: key={}, expected={}, got={}", 
    ...
    assert_eq!(total_conflicts, 0, "Should have zero data consistency violations");
...
async fn test_atomic_segment_router_coordination() {
                ...
                match rmi_clone.lookup(key) {
                    Some(value) => {
                        if value == expected_value {
                            ...
                            println!("Router consistency error: key={}, expected={}, got={}", 
    ...
    assert!(total_successes > 1500, "Should have many successful lookups, got {}", total_successes);
    ...
    assert!(error_rate < 0.05, "Router consistency error rate should be < 5%, got {:.2}%", error_rate * 100.0);
...
async fn test_concurrent_hot_buffer_merges() {
                        ...
                        Ok(_) => {
                            ...
                            if let Some(read_value) = rmi_clone.lookup(key) {
                                if read_value != value {
                                    ...
                                    eprintln!("Data integrity violation: key={}, expected={}, got={}", 
                                        key, value, read_value);
                                }
                            }
                        }
    ...
    assert_eq!(total_violations, 0, "Should have zero data integrity violations");
    
    // Merge conflicts should be rare
    let conflict_rate = total_conflicts as f64 / (total_successes + total_conflicts) as f64;
    assert!(conflict_rate < 0.1, "Merge conflict rate should be < 10%, got {:.2}%", conflict_rate * 100.0);
...
async fn test_stress_race_condition_comprehensive() {
    ...
    println!("  Operations/sec: {:.0}", total_ops as f64 / elapsed.as_secs_f64());
    ...
    assert!(race_rate < 0.001, "Race condition indicator rate should be < 0.1%, got {:.3}%", race_rate * 100.0);
    
    // System should remain performant
    let ops_per_sec = total_ops as f64 / elapsed.as_secs_f64();
    assert!(ops_per_sec > 1000.0, "Should maintain > 1000 ops/sec under stress");
