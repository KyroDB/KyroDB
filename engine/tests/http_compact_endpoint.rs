...
async fn http_compact_endpoint_smoke() {
    ...
    assert!(final_size < 100_000, "WAL size should be reasonable after compaction");
    
    // The compaction should process some data
    assert!(stats.before_bytes >= stats.after_bytes, "Compaction should not increase size");
}
