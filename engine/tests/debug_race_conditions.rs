...
async fn debug_simple_lookup_issue() {
    ...
    println!("📊 Stats after merge: segments={}, keys={}", stats.segment_count, stats.total_keys);
    ...
    for i in 0..100 {
        match rmi.lookup(i) {
            Some(value) => {
                ...
                if value != i * 10 {
                    println!("❌ Value mismatch for key {}: expected {}, got {}", i, i * 10, value);
                } else {
    ...
    println!("📈 Lookup results: {} successful, {} failed", successful_lookups, failed_lookups);
    ...
    println!("📈 Second lookup results: {} successful, {} failed", successful_lookups2, failed_lookups2);
    
    // Clean shutdown
    maintenance_handle.abort();
    
    let final_stats = rmi.get_stats();
    println!("📊 Final stats: segments={}, keys={}", final_stats.segment_count, final_stats.total_keys);
    
    // Basic assertions
    assert!(successful_lookups > 80, "Should have most lookups successful initially, got {}", successful_lookups);
    assert!(successful_lookups2 > 150, "Should have most lookups successful after background ops, got {}", successful_lookups2);
...
async fn debug_router_prediction() {
        ...
        match rmi.lookup(key) {
            Some(value) => {
                if value == expected {
                    ...
                    println!("❌ Hot buffer: key {} -> value {} (expected {})", key, value, expected);
                }
            }
            None => {
    ...
    let stats = rmi.get_stats();
    println!("📊 After merge: segments={}, keys={}", stats.segment_count, stats.total_keys);
        ...
        match rmi.lookup(key) {
            Some(value) => {
                if value == expected {
                    ...
                    println!("❌ Segment: key {} -> value {} (expected {})", key, value, expected);
                }
            }
            None => {
