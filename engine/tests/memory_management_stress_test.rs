...
async fn test_overflow_buffer_back_pressure() {
    ...
    println!("Final stats: hot_buffer={}, overflow={}, segments={}", 
    ...
    assert!(insert_count > 1000, "Should accept at least 1000 inserts before back-pressure");
...
async fn test_memory_circuit_breaker() {
    ...
    assert!(total_attempts > total_successes, "Should have some rejections due to memory protection");
    
    let final_stats = rmi.get_stats();
    println!("  Final memory state: hot={}, overflow={}, segments={}", 
...
async fn test_graduated_back_pressure_response() {
    ...
    for batch in 0..100 {
        ...
        for i in 0..100 {
            ...
            match rmi.insert(key, value) {
                ...
                Err(e) => {
                    ...
                    } else if error_msg.contains("permanently rejected") || error_msg.contains("critical") {
        ...
        if batch_errors > 50 {
            println!("Batch {}: {} errors out of 100 attempts", batch, batch_errors);
            if batch_errors >= 90 {
    ...
    assert!(pressure_levels_seen.len() > 0, "Should observe at least one pressure level");
    ...
    let final_stats = rmi.get_stats();
    println!("  Final state: hot={}, overflow={}, segments={}", 
        final_stats.hot_buffer_size, final_stats.overflow_size, final_stats.segment_count);
...
async fn test_memory_recovery_after_merge() {
    ...
    println!("After recovery: hot={}, overflow={}, segments={}", 
    ...
    let segments_increased = stats_after_recovery.segment_count > stats_before_recovery.segment_count;
    
    println!("Recovery indicators:");
    println!("  Overflow reduced: {} (from {} to {})", 
        overflow_reduced, stats_before_recovery.overflow_size, stats_after_recovery.overflow_size);
    println!("  Segments increased: {} (from {} to {})", 
    ...
    assert!(post_recovery_inserts > 10, "Should accept some writes after recovery");
    ...
    println!("  Data integrity: {}/1000 verified correctly", 1000 - verification_errors);
    assert!(verification_errors == 0, "Should have no data integrity violations");
...
async fn test_high_throughput_rag_ingestion_simulation() {
                    ...
                    Ok(_) => {
                        ...
                        if let Some(retrieved_hash) = rmi_clone.lookup(document_key) {
                            if retrieved_hash != embedding_hash {
                                eprintln!("Data corruption: key={}, expected={}, got={}", 
    ...
    let monitor_handle = tokio::spawn(async move {
        for _ in 0..30 {
            ...
            println!("Ingestion progress: {}/{} docs, hot={}, overflow={}, segments={}", 
    ...
    assert!(success_rate > 60.0, "Should achieve at least 60% success rate with back-pressure");
    assert!(final_successes > 1000, "Should successfully ingest at least 1000 documents");
    ...
    assert!(total_protection_events > 0, "Should have some memory protection activations");
    
    let final_stats = rmi.get_stats();
    println!("  Final system state: hot={}, overflow={}, segments={}", 
        final_stats.hot_buffer_size, final_stats.overflow_size, final_stats.segment_count);
    
    // System should not be completely exhausted
    assert!(final_stats.segment_count > 0, "Should have created segments from successful ingestions");
