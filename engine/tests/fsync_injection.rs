...
async fn append_recovery_with_injected_failures() {
    ...
    assert!(off >= 100, "recovered offset should be >= written count, got {off}");

    // Verify a few lookups exist (read-your-writes after recovery)
    for i in 0..10u64 {
        let (offset, _rec) = log2.find_key_scan(i).await.unwrap_or((0u64, kyrodb_engine::Record { key: 0, value: vec![] }));
        assert!(offset <= off);
    }
}
