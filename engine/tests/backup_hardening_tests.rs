...
use kyrodb_engine::backup::{
    BackupManager, BackupMetadata, BackupType, RestoreManager, RetentionPolicy, compute_backup_checksum,
};
use std::fs;
...
fn test_backup_metadata_handles_clock_errors() {
    ...
    let metadata2 = BackupMetadata::new_incremental(parent_id, 512, 50, 0x87654321, "Test inc".to_string());
...
fn test_pitr_incremental_chain_traversal() {
    ...
    assert_eq!(backups.iter().filter(|b| b.backup_type == BackupType::Full).count(), 1);
    assert_eq!(backups.iter().filter(|b| b.backup_type == BackupType::Incremental).count(), 3);
...
fn test_compute_backup_checksum() {
    ...
    writer.write_all(&(name1.len() as u32).to_le_bytes()).unwrap();
    writer.write_all(name1).unwrap();
    writer.write_all(&(data1.len() as u64).to_le_bytes()).unwrap();
    writer.write_all(data1).unwrap();
    
    // File 2: "data.bin" with content "Binary data"
    let name2 = b"data.bin";
    let data2 = b"Binary data";
    writer.write_all(&(name2.len() as u32).to_le_bytes()).unwrap();
    writer.write_all(name2).unwrap();
    writer.write_all(&(data2.len() as u64).to_le_bytes()).unwrap();
...
fn test_backup_create_and_verify_checksum() {
    ...
    // Create backup
    let manager = BackupManager::new(&backup_dir, &data_dir).unwrap();
    let metadata = manager.create_full_backup("Test backup".to_string()).unwrap();
...
fn test_prune_backups_respects_min_age() {
    ...
    assert!(!deleted.contains(&recent_id), "Recent backup should be protected by min_age_days");
}
