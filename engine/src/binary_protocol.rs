...
pub async fn binary_protocol_server(log: Arc<PersistentEventLog>, bind_addr: String) -> Result<()> {
...
async fn process_frame(mut frame: BytesMut, log: &Arc<PersistentEventLog>) -> Result<BytesMut> {
...
async fn process_put(mut payload: BytesMut, log: &Arc<PersistentEventLog>) -> Result<BytesMut> {
    let key = payload.get_u64_le();
...
async fn write_response(writer: &mut BufWriter<OwnedWriteHalf>, response: BytesMut) -> Result<()> {
