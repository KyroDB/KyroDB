...
mod tests {
    ...
    fn test_token_bucket_smooth_refill() {
        ...
        assert!(available >= 5.0 && available <= 20.0, "Expected ~10 tokens, got {}", available);
    }
    ...
    fn test_rate_limiter_concurrent_access() {
        ...
        assert!(total >= 45 && total <= 55, "Expected ~50 requests, got {}", total);
    }
    ...
    fn test_available_tokens() {
        ...
        let available = limiter.available_tokens("tenant_a").unwrap();
        assert!(available >= 48.0 && available <= 50.0, "Expected ~49 tokens, got {}", available);
    ...
    fn test_high_qps_precision() {
        ...
        assert!(available >= 5.0 && available <= 30.0, "Expected ~10 tokens, got {}", available);
    }
