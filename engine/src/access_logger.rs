...
//!
//! Architecture:
//! - Ring buffer: Fixed-size circular buffer (10M events, ~240MB)
//! - Lock-free writes: Single-writer, multiple-reader design
...
/// Ring buffer for access events with fixed capacity
///
/// Thread-safe design:
/// - Single writer (access logger)
/// - Multiple readers (training pipeline, metrics)
/// - Lock-free operations via atomics
///
/// # Memory Layout
/// - 10M events × 24 bytes = 240MB
/// - Circular buffer: oldest events overwritten
/// - Atomic head pointer for lock-free writes
#[derive(Debug)]
