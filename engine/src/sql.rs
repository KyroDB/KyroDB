...
fn extract_query_and_mode(
    expr: &Expr,
    out_query: &mut Option<Vec<f32>>,
    out_mode: &mut Option<String>,
) {
    if let Expr::BinaryOp { left, op, right } = expr {
        let op_s = op.to_string();
        if op_s == "=" {
            // Handle Identifier on either side of '='
            let try_side = |ident_expr: &Expr,
                            other: &Expr,
                            out_query: &mut Option<Vec<f32>>,
                            out_mode: &mut Option<String>| {
                if let Expr::Identifier(id) = ident_expr {
                    ...
                    } else if key == "MODE" && out_mode.is_none() {
    ...
    match &ast[0] {
        Statement::CreateTable { name, columns, .. } => {
            ...
            if lower == "vectors" || lower.ends_with("_vectors") || lower.contains("vectors") {
                ...
                let dim = if let Some(col) = columns.first() {
                    col.name.to_string().parse::<usize>().unwrap_or(0)
                } else {
                    0
                };
                if dim == 0 {
                    return Err(anyhow!("CREATE VECTORS requires dim INT as first column"));
                }
                reg.upsert_table(TableSchema {
                    name: tbl,
                    kind: TableKind::Vectors { dim },
                });
            } else {
                reg.upsert_table(TableSchema {
                    name: tbl,
        ...
        Statement::Insert {
            ...
            match &row[1] {
                Expr::Value(Value::SingleQuotedString(s)) => {
                    let offset = log
                        .append_kv(Uuid::new_v4(), key, s.clone().into_bytes())
                        .await?;
                ...
                Expr::Array(arr) => {
                    ...
                    for e in &arr.elem {
                        if let Expr::Value(Value::Number(n, _)) = e {
                            vec.push(n.parse::<f32>()?);
                        }
                    }
                    // Enforce dimension if registry has one
                    let reg = SchemaRegistry::load(&log.registry_path());
                    if let Some(dim) = reg.get_default_vectors_dim() {
                        if dim != vec.len() {
                            return Err(anyhow!("vector dim mismatch"));
                        }
                    }
...
async fn select_query(log: &PersistentEventLog, q: &Query) -> Result<SqlResponse> {
    let body = &q.body;
    ...
    if let Some(Expr::BinaryOp { left, op, right }) = &sel.selection {
        let mut query_vec: Option<Vec<f32>> = None;
        let mut mode: Option<String> = None;
        extract_query_and_mode(
            &Expr::BinaryOp {
                left: left.clone(),
                op: op.clone(),
                right: right.clone(),
        ...
        if let Some(query) = query_vec {
            ...
            let k = if let Some(Expr::Value(Value::Number(n, _))) = &q.limit {
            ...
            let res = if use_ann {
                log.search_vector_ann(&query, k).await
            } else {
                log.search_vector_l2(&query, k).await
            };
...
async fn select_by_key(log: &PersistentEventLog, q: &Query) -> Result<SqlResponse> {
    let body = &q.body;
    let SetExpr::Select(sel) = &**body else {
        return Err(anyhow!("only SELECT supported"));
    };
    ...
    let Some(key) = key_opt else {
        return Err(anyhow!("missing key in WHERE"));
    };
