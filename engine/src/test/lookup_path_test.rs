...
mod lookup_path_tests {
    ...
    async fn test_hot_buffer_immediate_visibility() {
        ...
        if !missing_keys.is_empty() {
            println!("Missing keys: {:?}", &missing_keys[..missing_keys.len().min(10)]);
        }
                ...
                Some(value) => {
                    if value == i * 100 {
                        ...
                        if mismatched_values <= 5 {
                            println!("Key {} returned wrong value: expected {}, got {}", i, i * 100, value);
                        }
                    }
                }
                None => {
        ...
        for writer_id in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                for i in 0..200 {
                    ...
                    rmi_clone.insert(key, key * 2).expect("Insert should succeed");
                }
            });
            ...
            match rmi.lookup_key_ultra_fast(i) {
                Some(value) => {
                    if value == i * 20 {
                        ...
                        wrong_values += 1;
                        if wrong_values <= 5 {
                            println!("Key {} returned old value: {} (expected {})", i, value, i * 20);
                        }
                    } else {
