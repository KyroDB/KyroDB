...
async fn test_wal_space_reclamation() {
    ...
    for i in 0..100 {
        let value = lookup_kv(&log, i).await.unwrap();
        assert!(value.is_some(), "Data lost after space reclamation for key {}", i);
    }
}
...
async fn test_wal_compaction_preserves_latest_values() {
    ...
    for key in 0..50 {
        ...
        assert!(value_str.contains("_k"), "Value format incorrect for key {}", key);
    }
}
    ...
    for worker_id in 0..3 {
        ...
        handles.spawn(async move {
            for i in 0..50 {
                ...
                append_kv(&log_clone, key, format!("concurrent_{}_{}", worker_id, i).into_bytes())
                    .await
                    .unwrap();
...
async fn test_wal_compaction_idempotent() {
    ...
    for i in 0..100 {
        let value = lookup_kv(&log, i).await.unwrap();
        assert!(value.is_some(), "Data lost after idempotent compaction for key {}", i);
    }
}
...
async fn test_wal_handles_rapid_snapshots() {
    ...
    for i in 0..200 {
        let value = lookup_kv(&log, i).await.unwrap();
        assert!(value.is_some(), "Data lost during rapid snapshots for key {}", i);
    }
}
...
async fn test_wal_compaction_performance() {
    ...
    for &key in &sample_keys {
        let value = lookup_kv(&log, key).await.unwrap();
        assert!(value.is_some(), "Data lost after performance compaction for key {}", key);
    }
}
