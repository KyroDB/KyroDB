...
use crate::{PersistentEventLog, GroupCommitConfig};
...
async fn test_debug_group_commit_with_custom_config() {
    ...
    match log.lookup_key(99).await {
        Some(found_offset) => {
            println!("  ✅ lookup_key(99) IMMEDIATELY returned offset={}", found_offset);
        }
        None => {
    ...
    match log.lookup_key(99).await {
        Some(found_offset) => {
            println!("  ✅ lookup_key(99) AFTER 10ms returned offset={}", found_offset);
        }
        None => {
...
async fn test_debug_group_commit_disabled() {
    ...
    match log.lookup_key(77).await {
        Some(found_offset) => {
            println!("  ✅ lookup_key(77) IMMEDIATELY returned offset={}", found_offset);
        }
        None => {
