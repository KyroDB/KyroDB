...
fn test_temp_data_dir_with_prefix() {
    ...
    assert!(
        name1.contains("test1"),
        "First directory should contain prefix 'test1'"
    );
    assert!(
        name2.contains("test2"),
        "Second directory should contain prefix 'test2'"
    );
}
...
async fn test_append_kv_ref() {
    ...
    let offset = append_kv_ref(&log, 100, b"ref_test".to_vec())
        .await
        .unwrap();
    // Offset can be 0 (valid), just check it succeeded
    let _ = offset;
...
fn test_to_path_conversion() {
    ...
    assert_eq!(
        path,
        dir.path(),
        "Converted path should match directory path"
    );
