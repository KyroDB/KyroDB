...
async fn test_test_server_start_default() {
    ...
    assert!(
        server.log.lookup_key(0).await.is_none(),
...
async fn test_test_server_basic_operations() {
    ...
    // Append some data
    append_kv(&server.log, 100, b"test_value".to_vec())
        .await
        .unwrap();
    append_kv(&server.log, 200, b"another_value".to_vec())
        .await
        .unwrap();
...
async fn test_test_server_with_async_durability() {
    ...
    for i in 0..20 {
        append_kv(
            &server.log,
            i * 10,
            format!("async_value_{}", i).into_bytes(),
        )
        .await
        .unwrap();
    }
    ...
    for (i, server) in cluster.iter().enumerate() {
        ...
        for (j, other_server) in cluster.iter().enumerate() {
            if i != j {
                ...
                assert!(
                    other_value.is_none(),
                    "Server {} should not have server {}'s data",
                    j,
                    i
                );
            }
