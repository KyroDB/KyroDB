...
async fn test_wait_for_eventual_success() {
    ...
    let result = wait_for(|| counter.load(Ordering::Relaxed) >= 5, 500).await;
...
async fn test_wait_for_timeout() {
    ...
    let result = wait_for(|| counter.load(Ordering::Relaxed) >= 100, 50).await;
...
async fn test_wait_for_with_concurrent_modification() {
    ...
    let result = wait_for(|| flag.load(Ordering::Relaxed) >= 5, 300).await;
...
async fn test_retry_with_backoff_zero_retries() {
    ...
    assert_eq!(
        attempts.load(Ordering::Relaxed),
        1,
        "Should only try once with 0 retries"
    );
}
...
async fn test_append_kv_concurrent_writers() {
    ...
    for worker in 0..10 {
        for i in 0..10 {
            ...
            assert!(
                value.is_some(),
                "Should find key from worker {} iteration {}",
                worker,
                i
            );
        }
