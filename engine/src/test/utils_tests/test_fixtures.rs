...
fn test_value_of_size() {
    ...
    assert_eq!(small.len(), SMALL_VALUE, "Small value should be correct size");
    
    let medium = value_of_size(MEDIUM_VALUE);
    assert_eq!(medium.len(), MEDIUM_VALUE, "Medium value should be correct size");
    
    let large = value_of_size(LARGE_VALUE);
    assert_eq!(large.len(), LARGE_VALUE, "Large value should be correct size");
}
...
fn test_dataset_generation_sequential() {
    ...
    assert_eq!(data.len(), config.key_count, "Should generate correct count");
    ...
    // Verify value sizes
    for (_, value) in &data {
        assert_eq!(value.len(), config.value_size, "All values should be correct size");
    }
}
