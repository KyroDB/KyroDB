...
impl MemoryManager {
    ...
    fn force_eviction(&self, needed_size: usize) -> Result<(), MemoryError> {
        ...
        let cache_freed = self
            .try_cache_eviction(needed_size)
            .map(|_| needed_size)
            .unwrap_or(0);
    ...
    fn try_buffer_pool_robust(
        ...
        if pressure == MemoryPressure::High {
            pool.enter_bypass_mode();
        } else if pressure == MemoryPressure::None {
            // Re-enable pool when pressure subsides
            pool.exit_bypass_mode();
        }
        ...
        if let Some(mut buffer) = pool.try_buffer_from_size(size) {
    ...
    fn update_memory_pressure(&self, current_bytes: usize) {
        ...
        self.memory_pressure
            .store(pressure as usize, Ordering::Relaxed);
    }
    ...
    pub fn stats(&self) -> MemoryStats {
        ...
        let (_small, _medium, _large, bypass_mode, pool_hits, pool_misses, pool_evictions) =
            pool.get_stats();
...
impl BufferPool {
    ...
    fn try_get_buffer(&mut self, min_size: usize) -> Option<Vec<u8>> {
        ...
        // Remove stale buffers that have not been used recently
        ...
        if let Some(index) = found_index {
            ...
            if let Some(timestamped_buffer) = pool.remove(index) {
                self.total_pooled = self
                    .total_pooled
                    .saturating_sub(timestamped_buffer.buffer.capacity());
                self.pool_hits += 1;
    ...
    fn return_buffer(&mut self, buffer: Vec<u8>) -> bool {
        let now = std::time::Instant::now();
        let capacity = buffer.capacity();

        if capacity == 0 || self.bypass_mode {
            return false; // Don't pool zero-capacity or when bypassing
        }
...
mod tests {
        ...
        match mgr.allocate(2048) {
            MemoryResult::Success(buffer) => {
                ...
                if !stats_before_realloc.pool_bypass_mode {
                    assert!(
                        stats_after_realloc.pool_hits > stats_before_realloc.pool_hits,
    ...
    fn test_buffer_pool_pressure_bypass() {
        ...
        assert!(matches!(
            mgr.memory_pressure(),
            MemoryPressure::High | MemoryPressure::Medium
        ));
        ...
        println!(
            "   💡  Memory pressure: {:?}, bypass mode: {}",
            stats_after.pressure, stats_after.pool_bypass_mode
        );
