...
impl TenantInfo {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        anyhow::ensure!(
            self.tenant_id.chars().all(|c| c.is_alphanumeric() || c == '_'),
        ...
        if !self.created_at.is_empty() {
            chrono::DateTime::parse_from_rfc3339(&self.created_at)
                .with_context(|| format!("created_at must be valid ISO 8601 timestamp: {}", self.created_at))?;
        }
...
mod tests {
    ...
    fn test_api_key_validation() {
        // Valid key
        assert!(AuthManager::validate_api_key("kyro_acme_a3f9d8e2c1b4567890abcdef12345678").is_ok());
        ...
        assert!(AuthManager::validate_api_key("kyro_acme_a3f9d8e2c1b4567890abcdef123456!!").is_err());
    }
    ...
    fn test_add_and_validate_key() {
        ...
        assert!(auth.validate("kyro_invalid_a3f9d8e2c1b4567890abcdef12345678").is_none());
    }
    ...
    fn test_list_tenants() {
        ...
        auth.add_key(create_test_api_key("acme"), create_test_tenant_info("acme")).unwrap();
        auth.add_key(create_test_api_key("startup"), create_test_tenant_info("startup")).unwrap();
...
"#;
    ...
    fn test_load_invalid_file() {
        ...
        // Non-existent file
        assert!(auth.load_from_file(Path::new("/nonexistent/path.yaml")).is_err());
...
"#;
        ...
        assert!(auth.validate("kyro_acme_a3f9d8e2c1b4567890abcdef12345678").is_some());
        ...
        assert!(auth.validate("kyro_acme_a3f9d8e2c1b4567890abcdef12345678").is_none());
        
        // New key is valid
        assert!(auth.validate("kyro_startup_b4e8f1a9d2c3567890fedcba98765432").is_some());
        assert_eq!(auth.key_count(), 1);
    }
}
