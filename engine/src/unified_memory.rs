...
impl<'a> Drop for ZeroAllocBuffer<'a> {
    fn drop(&mut self) {
        ...
        match self.size_category {
            BufferCategory::Small => {
                pool.small_available
                    .fetch_or(1 << self.buffer_id, Ordering::Relaxed);
            }
            BufferCategory::Medium => {
                pool.medium_available
                    .fetch_or(1 << self.buffer_id, Ordering::Relaxed);
            }
            BufferCategory::Large => {
                pool.large_available
                    .fetch_or(1 << self.buffer_id, Ordering::Relaxed);
            }
        }
    }
...
static GLOBAL_MEMORY_MANAGER: std::sync::OnceLock<UnifiedMemoryManager> =
...
macro_rules! with_zero_alloc_buffer {
    ($size:expr, $buf:ident, $code:block) => {
        if let Some(mut $buf) =
            crate::unified_memory::get_unified_memory().get_zero_alloc_buffer($size)
        {
            $code
        } else {
            // Fallback to emergency allocation if zero-alloc pool exhausted
            if let Some(emergency_buf) =
                crate::unified_memory::get_unified_memory().emergency_alloc($size)
            {
                let result = {
