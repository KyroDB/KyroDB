...
pub fn calculate_recall_at_k(results: &[RankingResult], k: usize, total_relevant: usize) -> f64 {
    ...
    let relevant_in_top_k = results.iter().take(k).filter(|r| r.relevance > 0.0).count();
...
pub fn calculate_mean_ndcg(query_results: &HashMap<u64, Vec<RankingResult>>, k: usize) -> f64 {
