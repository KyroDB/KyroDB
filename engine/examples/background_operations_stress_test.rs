...
async fn main() -> anyhow::Result<()> {
    ...
    let initial_data: Vec<(u64, u64)> = (0..10000).step_by(10).map(|i| (i, i * 2)).collect();
        ...
        async move {
            for iteration in 0..20 {
                ...
                if analytics.bounded_guarantee_ratio < 0.8 {
                    println!(
                        "⚠ WARNING: Bounded guarantee ratio dropped to {:.1}%",
    ...
    println!(
        "  Error rate: {:.4}%",
        (final_errors as f64 / final_writes as f64) * 100.0
    );
    ...
    for round in 0..5 {
        ...
        println!(
            "Round {}: Segments = {} (change: {:+})",
            round + 1,
            current_segments,
            current_segments as i32 - initial_segments as i32
        );
    }
    ...
    println!(
        "  Segments with bounded guarantee: {}/{} ({:.1}%)",
        comprehensive_analytics.segments_with_bounded_guarantee,
        comprehensive_analytics.total_segments,
        comprehensive_analytics.bounded_guarantee_ratio * 100.0
    );
    println!(
        "  Max search window observed: {}",
        comprehensive_analytics.max_search_window_observed
    );
    println!(
        "  Total lookups processed: {}",
        comprehensive_analytics.total_lookups
    );
    println!(
        "  Prediction errors: {}",
        comprehensive_analytics.total_prediction_errors
    );
    println!(
        "  Overall error rate: {:.4}%",
        comprehensive_analytics.overall_error_rate * 100.0
    );
    println!(
        "  Performance classification: {}",
        comprehensive_analytics.performance_classification
    );

    println!("\nSystem Validation:");
    println!(
        "  Meets all guarantees: {}",
        validation.system_meets_guarantees
    );
    println!(
        "  Worst-case complexity: {}",
        validation.worst_case_complexity
    );
    ...
    for (i, (stats, validation)) in comprehensive_analytics
        .segment_details
        .iter()
        .take(5)
        .enumerate()
    {
        println!(
            "  Segment {}: Size={}, Lookups={}, Errors={}, Window={}, Class={}",
            i,
            stats.data_size,
            stats.total_lookups,
    ...
    for batch in 0..10 {
        ...
        println!(
            "Batch {}: {:.2}ms, Segments: {}, Keys: {}, Hot: {:.1}%",
            batch + 1,
    ...
    println!(
        "Memory test completed in {:.2}s",
        total_memory_test_time.as_secs_f64()
    );
    ...
    println!(
        "Integrity check: {}/{} keys passed",
        test_keys.len() - integrity_errors,
        test_keys.len()
    );
    ...
    println!(
        "  Bounded guarantee: {:.1}%",
        final_analytics.bounded_guarantee_ratio * 100.0
    );
    println!(
        "  Performance class: {}",
        final_analytics.performance_classification
    );
