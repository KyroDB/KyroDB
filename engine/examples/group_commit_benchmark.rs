...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Test data directory
    let test_dir = std::path::PathBuf::from("/tmp/kyrodb_group_commit_benchmark");
    if test_dir.exists() {
        std::fs::remove_dir_all(&test_dir)?;
    }
    std::fs::create_dir_all(&test_dir)?;
    
    println!("🚀 KyroDB Group Commit Benchmark");
    println!("📂 Test directory: {}", test_dir.display());
    
    // Test configuration
    let test_writes = 10_000;
    ...
    let log = PersistentEventLog::open(&legacy_dir).await?;
    ...
    let legacy_throughput = test_writes as f64 / legacy_duration.as_secs_f64();
    ...
    env::set_var("KYRODB_GROUP_COMMIT_DELAY_MICROS", "100"); // 100µs max delay
    ...
    let log = PersistentEventLog::open(&group_commit_dir).await?;
    ...
    let group_commit_throughput = test_writes as f64 / group_commit_duration.as_secs_f64();
    ...
    if improvement > 5.0 {
        ...
        println!("   ⚠️ MODEST: {:.1}x performance improvement", improvement);
    } else {
