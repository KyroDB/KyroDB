...
fn main() -> Result<()> {
    ...
    for key in test_keys {
        ...
        if regular_result == fast_result {
            println!("  ✅ Key {}: Both methods returned {:?}", key, regular_result);
        } else {
            println!("  ❌ Key {}: lookup()={:?}, lookup_fast()={:?}", key, regular_result, fast_result);
    ...
    println!("    Hit rate: {:.2}%", (total_hits as f64 / total_ops as f64) * 100.0);
    ...
    println!("  📈 Single-threaded performance ({} iterations):", iterations);
    println!("    lookup():      {:?} ({:.0} ops/sec)", regular_duration, regular_ops_sec);
    println!("    lookup_fast(): {:?} ({:.0} ops/sec)", fast_duration, fast_ops_sec);
