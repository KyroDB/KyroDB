...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Generated {} Apple Silicon optimized key-value pairs",
        test_data.len()
    );
    ...
    for round in 0..neon_test_rounds {
        ...
        let test_indices: Vec<usize> = (0..4).map(|i| (round * 4 + i) % test_data.len()).collect();
        ...
        for (i, result) in neon_results.iter().enumerate() {
            if let Some(found_value) = result {
                ...
                if *found_value == expected_value {
                    ...
                    println!(
                        "❌ NEON validation failed for key {}: expected {}, got {}",
    ...
    println!(
        "📊 Sequential access: {:?} avg latency",
        sequential_duration / (cache_test_rounds * 4) as u32
    );
    println!(
        "📊 Random access: {:?} avg latency",
        random_duration / (cache_test_rounds * 4) as u32
    );
    println!(
        "📊 Cache efficiency ratio: {:.2}x",
    ...
    let memory_bandwidth = (memory_stress_rounds * 16 * 8) as f64
        / memory_stress_duration.as_secs_f64()
        / (1024.0 * 1024.0); // MB/s

    println!("✅ Unified memory stress test completed");
    println!(
        "📊 Memory bandwidth utilization: {:.2} MB/s",
        memory_bandwidth
    );
    println!(
        "📊 Average batch latency under stress: {:?}",
        memory_stress_duration / (memory_stress_rounds * 4) as u32
    );
