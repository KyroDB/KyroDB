...
async fn test_atomic_drain_consistency(rmi: Arc<AdaptiveRMI>) -> anyhow::Result<()> {
    ...
    println!("✅ Inserted {} entries in {:?}", total_inserted, insert_time);
    ...
    println!("✅ Data consistency verified: {}/{} entries found after atomic drain", 
             found_count, total_inserted);
    
    if found_count >= total_inserted * 8 / 10 { // Allow some loss due to back pressure
...
async fn test_hard_memory_limits(rmi: Arc<AdaptiveRMI>) -> anyhow::Result<()> {
    ...
    for i in 100_000..200_000u64 {
        ...
        match rmi.insert(i, i * 2) {
            ...
            Err(e) => {
                rejected_count += 1;
                if e.to_string().contains("HARD LIMIT") || e.to_string().contains("HARD CIRCUIT BREAKER") {
...
async fn test_memory_estimation_accuracy(_rmi: Arc<AdaptiveRMI>) -> anyhow::Result<()> {
    ...
    println!("📊 Memory estimation comparison for {} entries:", test_data.len());
...
async fn test_unbounded_growth_prevention(rmi: Arc<AdaptiveRMI>) -> anyhow::Result<()> {
    ...
    println!("   Bounded rejections: {} ({}%)", bounded_rejections, rejection_rate);
