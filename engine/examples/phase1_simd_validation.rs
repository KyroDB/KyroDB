...
fn main() {
    ...
    #[cfg(all(target_arch = "x86_64", target_feature = "avx2"))]
    {
        ...
        println!(
            "   • 8-key SIMD vs Scalar:  {}ns → {}ns ({:.1}x speedup)",
            scalar_baseline,
            simd_8_key,
            scalar_baseline as f64 / simd_8_key as f64
        );
        println!(
            "   • 16-key SIMD vs Scalar: {}ns → {}ns ({:.1}x speedup)",
            scalar_baseline,
            simd_16_key,
            scalar_baseline as f64 / simd_16_key as f64
        );
        println!(
            "   • Hot Buffer vs Linear:  {}ns → {}ns ({:.1}x speedup)",
            scalar_baseline,
            hot_buffer,
            scalar_baseline as f64 / hot_buffer as f64
        );
    }
    ...
    #[cfg(all(target_arch = "aarch64", target_feature = "neon"))]
    {
        ...
        println!(
            "   • 4-key NEON vs Scalar:  {}ns → {}ns ({:.1}x speedup)",
            scalar_baseline,
            neon_4_key,
            scalar_baseline as f64 / neon_4_key as f64
        );
        println!(
            "   • Hot Buffer vs Linear:  {}ns → {}ns ({:.1}x speedup)",
            scalar_baseline,
