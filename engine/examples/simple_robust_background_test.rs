...
async fn test_load_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Hot buffer: {:.1}% full", stats.hot_buffer_utilization * 100.0);
    ...
    match maintenance_result {
        Ok(()) => {
            ...
            if (elapsed.as_secs_f64() - expected_duration).abs() <= tolerance {
                println!("✅ Rate limiting working correctly (within {:.1}s tolerance)", tolerance);
            } else {
                println!("⚠️  Rate timing outside tolerance: {:.2}s vs expected {:.1}s ±{:.1}s", 
                        elapsed.as_secs_f64(), expected_duration, tolerance);
            }
            
            // Calculate observed rate
            let iterations = 300; // Current test limit
            let observed_rate = iterations as f64 / elapsed.as_secs_f64();
            println!("📊 Observed rate: {:.2} Hz (expected ~10 Hz)", observed_rate);
            
            if observed_rate <= 12.0 && observed_rate >= 8.0 {
                println!("✅ Rate is within acceptable bounds");
            } else {
                println!("⚠️  Rate outside acceptable bounds: {:.2} Hz", observed_rate);
            }
        }
        Err(e) => {
