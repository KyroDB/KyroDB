...
fn main() -> Result<(), Box<dyn std::error::Error>> {
                    ...
                    match rmi.insert(key, value) {
                        Ok(_) => {
                            ...
                            if let Some(retrieved) = rmi.lookup(key) {
                                if retrieved != value {
                                    eprintln!(
                                        "🚨 RACE CONDITION DETECTED: key={}, expected={}, got={}",
                                        key, value, retrieved
                                    );
                                ...
                                eprintln!(
                                    "🚨 INSERTION LOST: key={} was inserted but not found",
                                    key
                                );
                                error_counter.fetch_add(1, Ordering::Relaxed);
                            }
    ...
    if total_errors == 0 {
        ...
        println!(
            "   ❌ Concurrent insertion: {} RACE CONDITIONS DETECTED",
            total_errors
        );
    ...
    println!(
        "      - Throughput: {:.0} ops/sec",
        stress_total_operations as f64 / stress_duration.as_secs_f64()
    );

    let error_rate = (stress_total_errors as f64) / (stress_total_operations as f64) * 100.0;

    if error_rate < 1.0 {
        // Allow for very small error rate due to other factors
        println!(
            "   ✅ Stress test: ERROR RATE {:.2}% (ACCEPTABLE)",
            error_rate
        );
        println!("   🎯 Mixed operations stress test: PASSED");
    } else {
        println!(
            "   ❌ Stress test: ERROR RATE {:.2}% (TOO HIGH)",
            error_rate
        );
