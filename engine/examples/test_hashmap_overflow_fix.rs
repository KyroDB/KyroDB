...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &size in &test_sizes {
        ...
        println!(
            "  Insertions: {} entries in {:?} ({:.2} ops/sec)",
            size,
            insert_duration,
            size as f64 / insert_duration.as_secs_f64()
        );
        println!(
            "  Lookups: {} found out of 1000 in {:?} ({:.2} μs/lookup)",
            found_count,
            lookup_duration,
            lookup_duration.as_micros() as f64 / 1000.0
        );
        println!("  Single existing key lookup: {:?}", existing_key_time);
        println!(
            "  Single non-existing key lookup: {:?}",
            non_existing_key_time
        );
        ...
        if avg_lookup_micros > 50.0 {
            // More than 50 microseconds per lookup is suspicious
        ...
        println!(
            "  Buffer stats: {}/{} entries, {} rejected, pressure={}, ~{}MB",
