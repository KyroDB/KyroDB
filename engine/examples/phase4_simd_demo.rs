...
use kyrodb_engine::{KyroDb, get_ultra_fast_pool};
use uuid::Uuid;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let temp_dir = std::env::temp_dir().join(format!("kyrodb_simd_test_{}", 
...
async fn show_simd_capabilities(db: &KyroDb) {
    ...
    match db.get_simd_capabilities() {
        Some(capabilities) => {
            ...
            println!("⚡ AVX2 Support: {}", if capabilities.has_avx2 { "✅ YES" } else { "❌ NO" });
            println!("🚀 AVX512 Support: {}", if capabilities.has_avx512 { "✅ YES" } else { "❌ NO" });
            println!("� NEON Support: {}", if capabilities.has_neon { "✅ YES" } else { "❌ NO" });
...
async fn populate_test_data(db: &KyroDb) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📈 Insert Rate: {:.0} records/sec", num_records as f64 / duration.as_secs_f64());
...
async fn test_simd_batch_performance(db: &KyroDb) {
    ...
    println!("  💎 Avg Latency: {:.2}μs per lookup", duration.as_micros() as f64 / total_lookups as f64);
...
async fn test_batch_sizes(db: &KyroDb) {
    ...
    for &batch_size in &batch_sizes {
        ...
        println!("  📦 Batch Size {:3}: {:>10.0} lookups/sec ({:>6.2}μs/lookup)", 
                 batch_size, 
                 lookups_per_sec,
...
async fn compare_simd_vs_scalar(db: &KyroDb) {
    ...
    println!("🚀 SIMD Batch:   {:>10.0} lookups/sec ({:?})", simd_ops_per_sec, simd_duration);
    println!("🐌 Scalar Loop:  {:>10.0} lookups/sec ({:?})", scalar_ops_per_sec, scalar_duration);
...
async fn test_adaptive_batch_sizing(db: &KyroDb) {
    ...
    for (workload_name, keys) in workloads {
        ...
        let duration = start.elapsed();
        let hit_rate = results.iter().filter(|(_, v)| v.is_some()).count() as f64 / results.len() as f64;
        
        println!("  📊 {}: {} keys in {:?} (hit rate: {:.1}%)", 
                 workload_name, 
                 keys.len(), 
                 duration,
                 hit_rate * 100.0);
