...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_data: Vec<(u64, u64)> = (0..100000)
        .map(|i| {
            ...
            let value = key.wrapping_mul(0x9E3779B97F4A7C15).wrapping_add(0xDEADBEEF); 
            (key, value)
        })
        .collect();
    
    println!("✅ Generated {} enterprise test key-value pairs", test_data.len());
    ...
    println!("✅ Optimal batch size calculated: {} keys", optimal_batch_size);
    ...
    for &batch_size in &batch_sizes {
        ...
        println!("📊 Batch size {}: {:?} avg latency, {:.2} ops/sec", 
    ...
    #[cfg(all(target_arch = "x86_64", target_feature = "avx2"))]
    {
        ...
        for round in 0..avx2_test_rounds {
            ...
            let batch_keys: Vec<u64> = test_indices.iter()
                .map(|&idx| test_data[idx].0)
                .collect();
    ...
    for (pattern_name, stride) in cache_patterns {
        ...
        for i in 0..cache_test_rounds {
            let batch_keys: Vec<u64> = if stride == 0 {
                ...
                (0..16).map(|j| test_data[(i * 17 + j * 13) % test_data.len()].0).collect()
            } else {
                // Strided access pattern
                (0..16).map(|j| test_data[(i * stride + j * stride) % test_data.len()].0).collect()
            };
        ...
        println!("📊 {} access: {:?} avg latency, {:.2} ops/sec", 
    ...
    for &test_size in &memory_test_sizes {
        ...
        println!("📊 Dataset size {}: {:.2} ops/sec", test_size, memory_throughput);
    }
