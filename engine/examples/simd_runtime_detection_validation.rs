...
fn main() -> Result<()> {
    ...
    let individual_results: Vec<Option<u64>> =
        test_keys.iter().map(|&k| rmi.lookup_fast(k)).collect();
    ...
    for (i, ((&individual, &batch), &legacy)) in individual_results
        .iter()
        ...
        .enumerate()
    {
        if individual == batch && batch == legacy {
            ...
            println!(
                "  ❌ Mismatch at index {}: individual={:?}, batch={:?}, legacy={:?}",
    ...
    if correct_count == test_keys.len() {
        ...
        return Err(anyhow::anyhow!(
            "Correctness test failed - lookup methods disagree"
        ));
    }
    ...
    println!(
        "  📊 Performance Results ({} keys, {} iterations):",
        large_keys.len(),
        iterations
    );
    println!(
        "    Individual lookups:     {:?} ({:.0} ops/sec)",
        individual_avg,
        keys_per_test / individual_avg.as_secs_f64()
    );
    println!(
        "    Batch optimized:        {:?} ({:.0} ops/sec)",
        batch_optimized_avg,
        keys_per_test / batch_optimized_avg.as_secs_f64()
    );
    println!(
        "    Legacy batch SIMD:      {:?} ({:.0} ops/sec)",
        batch_simd_avg,
        keys_per_test / batch_simd_avg.as_secs_f64()
    );
    println!(
        "    Scalar baseline:        {:?} ({:.0} ops/sec)",
        scalar_optimized_avg,
        keys_per_test / scalar_optimized_avg.as_secs_f64()
    );
