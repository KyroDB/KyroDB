...
async fn main() -> anyhow::Result<()> {
    ...
    println!("📈 Average lookup time: {:?} per key", lookup_time / 100_000);
        ...
        let handle = thread::spawn(move || {
            ...
            for i in start_key..end_key {
                if let Some(value) = rmi_thread.lookup(i) {
                    assert_eq!(value, i * 2, "Race condition detected: value mismatch for key {}", i);
                    thread_found += 1;
                }
            }
            thread_found
    ...
    println!("✅ Concurrent lookup: found {}/100,000 keys in {:?}", total_concurrent_found, concurrent_time);
    println!("📈 Average concurrent lookup time: {:?} per key", concurrent_time / 100_000);
    ...
    if found_count == total_concurrent_found {
        ...
        println!("   Single-threaded: {}, Multi-threaded: {}", found_count, total_concurrent_found);
    }
...
fn main() {
    ...
    println!("Run with: cargo run --example race_free_lookup_verification --features learned-index");
}
