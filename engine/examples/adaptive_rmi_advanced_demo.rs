...
async fn main() -> anyhow::Result<()> {
    ...
    let initial_data: Vec<(u64, u64)> = (0..5000).map(|i| (i * 10, i * 100)).collect();
    ...
    for batch in 0..10 {
        ...
        println!(
            "Batch {}: Hot buffer: {}/{} ({:.1}%), Overflow: {}, Merge in progress: {}",
            batch + 1,
    ...
    println!(
        "  Hot buffer: {}, Overflow: {}",
        stats.hot_buffer_size, stats.overflow_size
    );
    ...
    println!(
        "Segments before adaptation: {}",
        analytics_before.total_segments
    );
    ...
    println!(
        "Segments after adaptation: {}",
        analytics_after.total_segments
    );
    ...
    println!(
        "  Bounded guarantee: {:.1}% segments",
        final_analytics.bounded_guarantee_ratio * 100.0
    );
    println!(
        "  Max search window: {} elements",
        final_analytics.max_search_window_observed
    );
    println!(
        "  Performance class: {}",
