...
fn test_corrected_simd_capabilities() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(all(target_arch = "aarch64", target_feature = "neon"))]
    {
        ...
        println!(
            "   ✅ Optimal batch size: {} (realistic for 2-wide SIMD)",
            capabilities.optimal_batch_size
        );
    }
...
fn test_honest_batch_sizing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(all(target_arch = "x86_64", target_feature = "avx2"))]
    {
        ...
        println!(
            "   ✅ AVX2 batch size: {} (realistic for 4-wide SIMD)",
            batch_size
        );
    }
    ...
    #[cfg(all(target_arch = "aarch64", target_feature = "neon"))]
    {
        ...
        println!(
            "   ✅ NEON batch size: {} (realistic for 2-wide SIMD)",
            batch_size
        );
    }
    ...
    )))]
    {
        ...
        println!(
            "   ✅ Scalar batch size: {} (conservative for scalar processing)",
            batch_size
        );
    }
...
fn test_realistic_simd_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   📊 Scalar lookup time: {:?} for 100 keys",
        scalar_duration
    );
    ...
    {
        assert!(
            speedup_ratio >= 1.0 && speedup_ratio <= 6.0,
            "❌ AVX2 speedup should be realistic: 1-6x, got {:.2}x",
            speedup_ratio
        );
        println!(
            "   ✅ AVX2 speedup: {:.2}x (realistic for 4-wide SIMD)",
            speedup_ratio
        );
    }
    ...
    {
        assert!(
            speedup_ratio >= 1.0 && speedup_ratio <= 4.0,
            "❌ NEON speedup should be realistic: 1-4x, got {:.2}x",
            speedup_ratio
        );
        println!(
            "   ✅ NEON speedup: {:.2}x (realistic for 2-wide SIMD)",
            speedup_ratio
        );
    }
    ...
    {
        assert!(
            speedup_ratio >= 0.8 && speedup_ratio <= 2.0,
            "❌ Scalar speedup should be minimal: 0.8-2x, got {:.2}x",
            speedup_ratio
        );
        println!(
            "   ✅ Scalar speedup: {:.2}x (minimal improvement from batching)",
            speedup_ratio
        );
    }
...
fn test_architectural_awareness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   📋 SIMD width: {} u64 values per register",
    ...
    if !capabilities.has_avx2 && !capabilities.has_neon {
        ...
        assert_eq!(
            capabilities.simd_width, 1,
            "Scalar should report 1-wide processing"
        );
    }
    ...
    match capabilities.architecture.as_str() {
        "x86_64" => {
            ...
            if capabilities.has_avx2 {
                assert!(
                    capabilities.optimal_batch_size % 8 == 0
                        || capabilities.optimal_batch_size <= 32,
        ...
        "aarch64" => {
            ...
            if capabilities.has_neon {
                assert!(
                    capabilities.optimal_batch_size % 4 == 0
                        || capabilities.optimal_batch_size <= 16,
        ...
        _ => {
            ...
            assert!(
                capabilities.optimal_batch_size <= 64,
