...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   📊 Initial SIMD batch size: {}", initial_capabilities.optimal_batch_size);
    ...
    println!("      - Results: {}/100", batch_results.iter().filter(|r| r.is_some()).count());
    ...
    let start = Instant::now();
    let lookup_results: Vec<_> = large_dataset.iter()
        .take(1000)
