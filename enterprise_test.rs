...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // EnterpriseSafe Group Commit Configuration
    let config = GroupCommitConfig {
        max_batch_size: 100,       // Batch up to 100 writes
        max_delay: Duration::from_micros(200), // 200µs max delay
        durability: DurabilityLevel::EnterpriseSafe, // Zero data loss mode
    };
    
    let wal_path = test_dir.join("enterprise.wal");
    let kyrodb = Arc::new(
    ...
    for i in 0..test_writes {
        ...
        let key = format!("enterprise_key_{}", i);
        let value = serde_json::json!({
            "id": i,
            "data": "x".repeat(test_value_size),
    ...
    let test_key = "enterprise_key_0";
    if let Some(_value) = kyrodb.lookup(test_key).await? {
