...
curl -s http://127.0.0.1:51051/metrics \
  | grep -E 'kyrodb_cache_(hits|misses|hit_rate|size)|kyrodb_training_cycles_completed_total'
```

## Key Contracts

- L1a is a document cache (`VectorCache`) behind a pluggable `CacheStrategy` (`LRU` or learned predictor).
...
Important nuance:
- `/metrics` currently exports aggregate cache counters; detailed L1a/L1b split is available via validation harness stats (`TieredEngineStats`) and validation artifacts, not as separate Prometheus counters.
...
- L1b entries are cleared on hot-tier flush paths that can change search neighborhoods.
